...
- **Phases 3–6 (automatic):** Build (TDD — reuse the Stage 1 feedback loop as the failing test wherever possible), validate, converge (`/review-cycle` → push → open draft PR), and judge with adversarial test lenses. No further pauses; ends at an open draft PR for human merge.
