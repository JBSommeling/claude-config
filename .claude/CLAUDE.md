...
All four accept an optional `repos=<path>` parameter. A single path is supported in v1: relative paths resolve against `$HOME`, absolute and `~/` paths are used as-is. The primary repo is always the current working directory; siblings are direct children of that path holding a `.git` entry. When present, the spec allocates work across repos, the plan tags tasks per repo, and build and PR phases run per repo — PRs dispatch to `gh` for GitHub or `az repos` for Azure DevOps. Absent the parameter, behaviour is unchanged.
